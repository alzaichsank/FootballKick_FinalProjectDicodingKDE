package alzaichsank.com.aplikasifootbalmatchschedule.view.search.match.adapter
...
class SearchAdapter(private val onClickListener: (position: Int) -> Unit) : BaseAdapter<RecyclerView.ViewHolder, EventsItem>() {
...
class ProductViewHolder(viewItem: View, onClickListener: (position: Int) -> Unit) : RecyclerView.ViewHolder(viewItem) {
    ...
    fun bindData(data: EventsItem, position: Int) {
            ...
            if(data.intHomeScore == null) {
                ...
                if (data.strTime.equals("00:00:00+00:00")||data.strTime.toString().toLowerCase().contains("null")) {
                    itemView.time.text = "-"
                    ...
                    itemView.time.text = DateTime.getBasicTime(DateTime.toGMTFormat(data.dateEvent.toString(), data.strTime.toString()+":00+00:00"))
                }  else {

                    itemView.time.text = DateTime.getBasicTime(DateTime.toGMTFormat(data.dateEvent.toString(), data.strTime.toString()+"+00:00"))
                }
